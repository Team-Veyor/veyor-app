...
const DefaultToastIcon = ({ type }: { type: ToastType }) => {
  if (type === 'success') {
    return <CheckCircleIcon className={cn('size-24', ICON_CLASSES[type])} />;
  }

  return <AlertCircleIcon className={cn('size-24', ICON_CLASSES[type])} />;
};
...
export const Toast = ({ type = 'warning', message, icon, className, role }: ToastProps) => {
  return (
    <motion.div
      ...
      className={cn(
        'inline-flex transform-gpu items-center gap-8 rounded-16 bg-black-alpha-30 px-16 py-12 text-white backdrop-blur-lg',
        className,
      )}
    >
      {icon ?? <DefaultToastIcon type={type} />}
      <span className='label-small whitespace-nowrap'>{message}</span>
    </motion.div>
  );
};
