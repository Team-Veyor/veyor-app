...
const SURVEY_COLS =
  'id, title, external_url, reward_amount, est_minutes, target_gender, target_birth_year_min, target_birth_year_max, opens_at, expires_at';
...
export class SurveysRepository {
  ...
  async getProfileTarget(
    userId: string,
  ): Promise<{ birth_year: number | null; gender: string | null }> {
    const { data } = await this.db
      .from('profiles')
      .select('birth_year, gender')
      .eq('id', userId)
      .maybeSingle();
    return { birth_year: data?.birth_year ?? null, gender: data?.gender ?? null };
  }
}
